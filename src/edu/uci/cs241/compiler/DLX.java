...
////		DLX.load(compiler.changeType(
////		compiler.readFileAsString("D:/compiler/gc0.s").toCharArray()));
////		DLX.load(compiler.changeType("ADDI 2,2,2".toCharArray()));
//		int[] a = new int[4];
//		a[0] = compiler.makeTupleF1(32, 4, 0, 12);
//		a[1] = compiler.makeTupleF1(0, 4, 4, 4);
//		a[2] = compiler.makeTupleF2(51, 0, 4, 0);
//		a[3] = 100;
//		DLX.load(a);
//		DLX.execute();
	}
	...
	public static void execute() throws IOException {
		...
		while (true) {
			...
			switch (op) {
				...
				case ADDI:
					R[a] = R[b] + c;
					//System.out.println("add R["+a+"]="+R[a]+" R["+b+"]="+R[b]+" + "+c);
					break;
				case SUB:
				case SUBI:
					R[a] = R[b] - c;
					//System.out.println("sub R["+a+"]="+R[a]+" R["+b+"]="+R[b]+" - "+c);
					break;
				case CMP:
				case CMPI:
					//System.out.println("cmp "+" R["+b+"]="+R[b]+" c="+c);
				...
				case MUL:
				case MULI:
					//System.out.println("mul R["+a+"]="+R[a]+" R["+b+"]="+R[b]+" * "+c);
					R[a] = R[b] * c;
					break;
				case DIV:
				case DIVI:
					//System.out.println("div R["+a+"]="+R[a]+" R["+b+"]="+R[b]+" / "+c);
					R[a] = R[b] / c;
					break;
				case MOD:
				case MODI:
				...
				case LDX: // remember: c == R[origc] because of F2 format
					R[a] = M[(R[b]+c) / 4];
					//System.out.println("ldx to "+R[a]+" from R["+b+"] "+(R[b]) / 4+" + "+c/4+"="+((R[b]+c) / 4));
					break;
				case STW:
				case STX: // remember: c == R[origc] because of F2 format
					M[(R[b]+c) / 4] = R[a]; 
					//System.out.println("stx  "+R[a]+" to R["+b+"] "+(R[b]) / 4+" + "+c/4+"="+((R[b]+c) / 4));
					break;
				case POP:
					R[a] = M[R[b] / 4];
					R[b] = R[b] + c;
				...
				case BEQ:
					...
					//System.out.println("beq "+R[a]+" "+nextPC);
					break;
				case BNE:
					if (R[a] != 0) nextPC = PC + c;
					if ((nextPC < 0) || (nextPC > MemSize/4)) {
					...
					//System.out.println("bne "+R[a]+" "+nextPC);
					break;
				case BLT:
					if (R[a] < 0) nextPC = PC + c;
					if ((nextPC < 0) || (nextPC > MemSize/4)) {
					...
					//System.out.println("blt "+R[a]+" "+nextPC);
					break;
				case BGE:
					if (R[a] >= 0) nextPC = PC + c;
					if ((nextPC < 0) || (nextPC > MemSize/4)) {
					...
					//System.out.println("bge "+R[a]+" "+nextPC);
					break;
				case BLE:
					if (R[a] <= 0) nextPC = PC + c;
					if ((nextPC < 0) || (nextPC > MemSize/4)) {
					...
					//System.out.println("ble "+R[a]+" "+nextPC);
					break;
				case BGT:
					if (R[a] > 0) nextPC = PC + c;
					if ((nextPC < 0) || (nextPC > MemSize/4)) {
					...
					//System.out.println("bgt "+R[a]+" "+nextPC);
					break;
				case BSR:
					R[31] = (PC+1) * 4;
					nextPC = PC + c;
					//System.out.println("bsr"+nextPC);
					break;
				case JSR:
					R[31] = (PC+1) * 4;
					nextPC = c / 4;
